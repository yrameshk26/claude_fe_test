...
import { getTransactionFilters } from '$lib/utils/filters'
import {
...
export default async function load({
	...
	try {
		// handle filtering
		const searchTerms = getTransactionFilters()
		...
		const [data, errors] = await graphqlFetchAction({
			fetch: fetchFunction,
			query: `
		query GET_ALL_TRANSACTIONS($where: TransactionWhereInput, $orderBy: [TransactionOrderByInput!], $skip: Int, $take: Int, $persistentWhere: TransactionWhereInput) {
			getAllTransactions(where: $where, orderBy: $orderBy, skip: $skip, take: $take, persistentWhere: $persistentWhere)
		}
	`,
			variables: {
				where,
		...
		// get summary
		const summary = {
			filterCount: data.getAllTransactions.metrics.filteredCount,
			totalCount: data.getAllTransactions.metrics.allCount
		}
		...
		return {
			transactions: data.getAllTransactions.data,
			summary,
			searchTerms
		}
	} catch (error) {
