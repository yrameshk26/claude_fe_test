...
import { getStorePhoneNumberFilters } from '$lib/utils/filters'
import {
...
export default async function load({
	...
	try {
		// handle filtering
		const searchTerms = getStorePhoneNumberFilters()
		...
		const [data, errors] = await graphqlFetchAction({
			fetch: fetchFunction,
			query: `
		query GET_ALL_STORE_PHONE_NUMBERS($where: StorePhoneNumberWhereInput, $orderBy: [StorePhoneNumberOrderByInput!], $skip: Int, $take: Int, $persistentWhere: StorePhoneNumberWhereInput) {
			getAllStorePhoneNumbers(where: $where, orderBy: $orderBy, skip: $skip, take: $take, persistentWhere: $persistentWhere)
		}
	`,
			variables: {
				where,
		...
		// get summary
		const summary = {
			filterCount: data.getAllStorePhoneNumbers.metrics.filteredCount,
			totalCount: data.getAllStorePhoneNumbers.metrics.allCount
		}
		...
		return {
			storePhoneNumbers: data.getAllStorePhoneNumbers.data,
			summary,
			searchTerms
		}
	} catch (error) {
