...
import { getCallCategoryLevelThreeFilters } from '$lib/utils/filters'
import {
...
export default async function load({
	...
	try {
		// handle filtering
		const searchTerms = getCallCategoryLevelThreeFilters()
		...
		const [data, errors] = await graphqlFetchAction({
			fetch: fetchFunction,
			query: `
		query GET_ALL_CALL_CATEGORIES_LEVEL_THREE($where: CallCategoryLevelThreeWhereInput, $orderBy: [CallCategoryLevelThreeOrderByInput!], $skip: Int, $take: Int, $persistentWhere: CallCategoryLevelThreeWhereInput) {
			getAllCallCategoriesLevelThree(where: $where, orderBy: $orderBy, skip: $skip, take: $take, persistentWhere: $persistentWhere)
		}
	`,
			variables: {
				where,
		...
		// get summary
		const summary = {
			filterCount: data.getAllCallCategoriesLevelThree.metrics.filteredCount,
			totalCount: data.getAllCallCategoriesLevelThree.metrics.allCount
		}
		...
		return {
			callCategoriesLevelThree: data.getAllCallCategoriesLevelThree.data,
			summary,
			searchTerms
		}
	} catch (error) {
