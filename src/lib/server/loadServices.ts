...
export default async function load({
	...
	try {
		...
		const [data, errors] = await graphqlFetchAction({
			fetch: fetchFunction,
			query: `
		query GET_ALL_SERVICES($where: ServiceWhereInput, $orderBy: [ServiceOrderByInput!], $skip: Int, $take: Int, $persistentWhere: ServiceWhereInput) {
			getAllServices(where: $where, orderBy: $orderBy, skip: $skip, take: $take, persistentWhere: $persistentWhere)
		}
	`,
			variables: {
				where,
		...
		// get summary
		const summary = {
			filterCount: data.getAllServices.metrics.filteredCount,
			totalCount: data.getAllServices.metrics.allCount
		}
		...
		return {
			services: data.getAllServices.data,
			summary,
			searchTerms
		}
	} catch (error) {
