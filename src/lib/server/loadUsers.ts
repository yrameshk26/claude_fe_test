...
import { getUserFilters } from '$lib/utils/filters'
import {
...
export default async function load({
	...
	try {
		// handle filtering
		const searchTerms = getUserFilters()
		...
		const sortTerms = {
			...
			isAdmin: null,
			permissions: '_count',
			requestedLoginApprovals: '_count',
			assignedToTickets: '_count'
		}
		const sortBy = sortTermsToPrismaQuery(url, sortTerms) as Record<string, unknown>[]
		if (sortBy.length === 0) {
			sortBy.push({ isAdmin: `desc` })
			sortBy.push({ permissions: { _count: `desc` } })
		...
		const [data, errors] = await graphqlFetchAction({
			fetch: fetchFunction,
			query: `
		query GET_ALL_CUSTOMERS($where: UserWhereInput, $orderBy: [UserOrderByInput!], $skip: Int, $take: Int, $persistentWhere: UserWhereInput) {
			getAllUsers(where: $where, orderBy: $orderBy, skip: $skip, take: $take, persistentWhere: $persistentWhere)
		}
	`,
			variables: {
				where,
		...
		// get summary
		const summary = {
			filterCount: data.getAllUsers.metrics.filteredCount,
			totalCount: data.getAllUsers.metrics.allCount
		}
