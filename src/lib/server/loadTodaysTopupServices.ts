...
export default async function load({
	...
	try {
		...
		delete searchTerms.serviceTypeID
		delete searchTerms.serviceType
		...
		if (sortBy.length === 0) {
			sortBy.push({ expiryDate: `asc` })
			sortBy.push({ credits: `asc` })
			sortBy.push({ customer: { fullName: `asc` } })
		}
		...
		const [data, errors] = await graphqlFetchAction({
			fetch: fetchFunction,
			query: `
		query GET_TODAYS_TOPUP_SERVICES($serviceTypeID: String!, $where: ServiceWhereInput, $orderBy: [ServiceOrderByInput!], $skip: Int, $take: Int, $persistentWhere: ServiceWhereInput) {
			getTodaysTopupServices(serviceTypeID: $serviceTypeID, where: $where, orderBy: $orderBy, skip: $skip, take: $take, persistentWhere: $persistentWhere)
		}
	`,
			variables: {
				serviceTypeID,
		...
		// get summary
		const summary = {
			filterCount: data.getTodaysTopupServices.metrics.filteredCount,
			totalCount: data.getTodaysTopupServices.metrics.allCount
		}
		...
		return {
			services: data.getTodaysTopupServices.data,
			summary,
			searchTerms
		}
	} catch (error) {
