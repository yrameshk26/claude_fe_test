...
import { getCallCategoryLevelOneFilters } from '$lib/utils/filters'
import {
...
export default async function load({
	...
	try {
		// handle filtering
		const searchTerms = getCallCategoryLevelOneFilters()
		...
		const [data, errors] = await graphqlFetchAction({
			fetch: fetchFunction,
			query: `
		query GET_ALL_CALL_CATEGORIES_LEVEL_ONE($where: CallCategoryLevelOneWhereInput, $orderBy: [CallCategoryLevelOneOrderByInput!], $skip: Int, $take: Int, $persistentWhere: CallCategoryLevelOneWhereInput) {
			getAllCallCategoriesLevelOne(where: $where, orderBy: $orderBy, skip: $skip, take: $take, persistentWhere: $persistentWhere)
		}
	`,
			variables: {
				where,
		...
		// get summary
		const summary = {
			filterCount: data.getAllCallCategoriesLevelOne.metrics.filteredCount,
			totalCount: data.getAllCallCategoriesLevelOne.metrics.allCount
		}
		...
		return {
			callCategoriesLevelOne: data.getAllCallCategoriesLevelOne.data,
			summary,
			searchTerms
		}
	} catch (error) {
