...
import { getPaymentMethodFilters } from '$lib/utils/filters'
import {
...
export default async function load({
	...
	try {
		// handle filtering
		const searchTerms = getPaymentMethodFilters()
		...
		const [data, errors] = await graphqlFetchAction({
			fetch: fetchFunction,
			query: `
		query GET_ALL_PAYMENT_METHODS($where: PaymentMethodWhereInput, $orderBy: [PaymentMethodOrderByInput!], $skip: Int, $take: Int, $persistentWhere: PaymentMethodWhereInput) {
			getAllPaymentMethods(where: $where, orderBy: $orderBy, skip: $skip, take: $take, persistentWhere: $persistentWhere)
		}
	`,
			variables: {
				where,
		...
		// get summary
		const summary = {
			filterCount: data.getAllPaymentMethods.metrics.filteredCount,
			totalCount: data.getAllPaymentMethods.metrics.allCount
		}
		...
		return {
			paymentMethods: data.getAllPaymentMethods.data,
			summary,
			searchTerms
		}
	} catch (error) {
