...
import { getServiceTypeUrlFilters } from '$lib/utils/filters'
import {
...
export default async function load({
	...
	try {
		// handle filtering
		const searchTerms = getServiceTypeUrlFilters()
		...
		const [data, errors] = await graphqlFetchAction({
			fetch: fetchFunction,
			query: `
		query GET_ALL_SERVICE_TYPE_URLS($where: ServiceTypeUrlWhereInput, $orderBy: [ServiceTypeUrlOrderByInput!], $skip: Int, $take: Int, $persistentWhere: ServiceTypeUrlWhereInput) {
			getAllServiceTypeUrls(where: $where, orderBy: $orderBy, skip: $skip, take: $take, persistentWhere: $persistentWhere)
		}
	`,
			variables: {
				where,
		...
		// get summary
		const summary = {
			filterCount: data.getAllServiceTypeUrls.metrics.filteredCount,
			totalCount: data.getAllServiceTypeUrls.metrics.allCount
		}
		...
		return {
			serviceTypeUrls: data.getAllServiceTypeUrls.data,
			summary,
			searchTerms
		}
	} catch (error) {
