...
export async function load({ locals }) {
	if (
		...
		!locals.session.permissions.find(
			({ permissionType }) => permissionType === 'SEPARATE_CUSTOMERS'
		)
	) {
...
export const actions = {
	default: async ({ request, fetch, locals }) => {
		...
		const [result, errors] = await graphqlFetchAction({
			fetch,
			query: `
        mutation CREATE_CUSTOMER_FROM_SERVICE($serviceID: String!, $data: CreateCustomerInput!) {
          createCustomerFromService(serviceID: $serviceID, data: $data)
        }
      `,
			variables: {
		...
		return result.createCustomerFromService
	}
} satisfies Actions
