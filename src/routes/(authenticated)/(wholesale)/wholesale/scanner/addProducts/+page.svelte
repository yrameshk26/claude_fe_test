<script lang="ts">
	...
	headerState.title = 'Scan New Products'
	...
	let duplicateErrorProducts = $state([]) as { macID?: string; serialNo: string }[]
	...
	const onSubmit: SubmitFunction = async ({ formData }) => {
		...
		return async ({ result, update }) => {
			...
			if (result.type === 'failure') {
				...
				duplicateErrorProducts = result.data?.detailedErrors[0]?.extensions?.existingProducts || []
...
>
	...
	<div class="grid grid-cols-1 gap-8 p-4 lg:h-[calc(100%-36px)] lg:grid-cols-5">
		<Table classContainer="lg:col-span-3 max-h-96 lg:max-h-[calc(100%)] lg:w-fit">
			...
			{#snippet tableRows()}
				{#each scannedProducts as product (product.serialNo)}
					<TableCellRow
						class={twMerge(
							duplicateErrorProducts?.find((p) => p.serialNo === product.serialNo) &&
		...
		<div class="flex flex-col gap-4 overflow-auto p-4 lg:col-span-2 lg:max-h-[calc(100%)] lg:gap-8">
			<div class="flex flex-col gap-4">
				...
				<div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
					...
					<DateInput label="Purchase Date" name="vendorPurchaseDt" isRequired value={new Date()} />
					<DateInput
        ...
        <div class="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <DateInput label="Purchase Date" name="resellerPurchaseDt" />
          <DateInput label="Warranty Expiry Date" name="resellerWarrantyExpiryDt" />
        </div>
      </div> -->
		</div>
	</div>
