<script lang="ts">
	...
	headerState.title = 'Payments Report'
	...
	const GROUP_BY_OPTIONS = [
		...
		{ label: 'Terminal', value: 'terminalID' },
	...
	<Table classContainer="my-4 h-[calc(100%-248px)] lg:h-[calc(100%-98px)]">
		{#snippet tableHead()}
			<TableHeadRow>
				...
				<TableHead class="min-w-36 text-xs uppercase">Amount</TableHead>
				<TableHead class="min-w-0 ">
					<TablePrint name={`Payments from ${fromDate} to ${toDate} by ${groupBy.join(',')}`} />
					<SaveToQuickReport />
				</TableHead>
			</TableHeadRow>
		{/snippet}
