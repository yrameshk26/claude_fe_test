...
import { getPaymentFilters } from '$lib/utils/filters'
...
export async function load({ fetch, url }) {
	...
	const searchTerms = getPaymentFilters()
	...
	const [data, errors] = await graphqlFetchAction({
		fetch,
		query: `
			query CUSTOMER_REPORT_V2($startDate: String!, $endDate: String!, $groupByFilter: [String], $whereFilter: PaymentWhereInput) {
        paymentReportV2(startDate: $startDate, endDate: $endDate, groupByFilter: $groupByFilter, whereFilter: $whereFilter)
			}
		`,
		variables: {
			startDate: from,
