...
export async function load({ fetch, url }) {
	...
	const [data, errors] = await graphqlFetchAction({
		fetch,
		query: `
			query REMAINING_TOPUPS_REPORT_V2($startDate: String!, $endDate: String!, $groupByFilter: [String]) {
        remainingTopupsReportV2(startDate: $startDate, endDate: $endDate, groupByFilter: $groupByFilter)
			}
		`,
		variables: {
			startDate: from,
	...
	return {
		...
		groupBy,
		searchTerms,
		report: data.remainingTopupsReportV2
	}
}
