<script lang="ts">
	...
	headerState.title = 'Services Report'
	...
	const GROUP_BY_OPTIONS = [
		...
		{ label: 'Service Type Url', value: 'serviceTypeUrlID' },
		{ label: 'Credits', value: 'credits' },
		{ label: 'Put On Hold', value: 'putOnHold' },
		{ label: 'Inactive', value: 'inactive' },
		{ label: 'Is Monthly', value: 'isMonthly' },
		{ label: 'Is From Call', value: 'isFromCall' },
		{ label: 'Is Switched', value: 'isSwitched' },
		...
		{#snippet tableHead()}
			<TableHeadRow>
				...
				<TableHead class="min-w-0 ">
					<TablePrint name={`Services from ${fromDate} to ${toDate} by ${groupBy.join(',')}`} />
					<SaveToQuickReport />
				</TableHead>
			</TableHeadRow>
		{/snippet}
