<script lang="ts">
	...
	headerState.title = 'Call Records Report'
	...
	const GROUP_BY_OPTIONS = [
		...
		{ label: 'Wait Time', value: 'customerWaitSeconds' },
		{ label: 'Call Type', value: 'callType' },
		{ label: 'Is New Customer', value: 'isNewCustomer' },
		{ label: 'Service Category', value: 'callCategoryLevelOneID' },
		{ label: 'Main Category', value: 'callCategoryLevelTwoID' },
		{ label: 'Sub Category', value: 'callCategoryLevelThreeID' },
		{ label: 'Store Phone', value: 'storePhoneNumberID' },
		{ label: 'Product Type', value: 'productTypeID' },
	...
	<Table classContainer="my-4 h-[calc(100%-248px)] lg:h-[calc(100%-98px)]">
		{#snippet tableHead()}
			<TableHeadRow>
				...
				<TableHead class="min-w-36 text-xs uppercase">Duration</TableHead>
				<TableHead class="min-w-0">
					<TablePrint name={`CallRecords from ${fromDate} to ${toDate} by ${groupBy.join(',')}`} />
					<SaveToQuickReport />
				</TableHead>
			</TableHeadRow>
		{/snippet}
