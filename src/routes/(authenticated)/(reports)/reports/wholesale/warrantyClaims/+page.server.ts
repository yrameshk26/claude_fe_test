...
export async function load({ fetch, url }) {
	...
	const [data, errors] = await graphqlFetchAction({
		fetch,
		query: `
			query CALL_RECORD_REPORT_V2($startDate: String!, $endDate: String!, $groupByFilter: [String], $whereFilter: WarrantyClaimWhereInput) {
        warrantyClaimReportV2(startDate: $startDate, endDate: $endDate, groupByFilter: $groupByFilter, whereFilter: $whereFilter)
			}
		`,
		variables: {
			startDate: from,
	...
	return {
		...
		groupBy,
		searchTerms,
		report: data.warrantyClaimReportV2
	}
}
