...
export async function load({ fetch, url }) {
	...
	const [data, errors] = await graphqlFetchAction({
		fetch,
		query: `
			query CUSTOMER_REPORT_V2($startDate: String!, $endDate: String!, $groupByFilter: [String], $whereFilter: ActivityWhereInput) {
        activityReportV2(startDate: $startDate, endDate: $endDate, groupByFilter: $groupByFilter, whereFilter: $whereFilter)
			}
		`,
		variables: {
			startDate: from,
