<script lang="ts" module>
	...
	import type { Props as DateTimeInputProps } from '../inputs/DateTimeInput.svelte'
	import type { SubmitFunction } from '@sveltejs/kit'

	export type Props = DateTimeInputProps & {
		action: string
...
<script lang="ts">
	...
	let parsedDateISOValue = $derived(
	...
	$effect(() => {
		...
		const stopScrolling = (e: Event) => {
			if (formState.value !== parsedDateISOValue) {
				e.preventDefault()
			}
		}
		if (formState.value === parsedDateISOValue) return
	...
	let formState = $state({
		...
		errors: [] as string[],
		reset: () => {
			formState.value = new Date(value).toLocaleString('en-CA', {
				year: 'numeric',
				month: '2-digit',
				day: '2-digit',
				hour: '2-digit',
		...
		<div class="flex items-center gap-1 text-sm">
			<div id={containerID} class={twMerge('w-full', classValue)}>
				...
				{:else}
					...
					{/if}
					{#if !value}
						<span class="text-xs text-gray-300"> YYYY-MM-DD </span>
					{/if}
				{/if}
			</div>
			...
			>
				...
				>
					...
					<SolidButton
						...
						isDisabled={formState.value === parsedDateISOValue}
					>
						{#if !formState.isLoading}
							<CheckIcon />
						{/if}
