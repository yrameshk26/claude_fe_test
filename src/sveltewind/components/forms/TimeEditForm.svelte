<script lang="ts" module>
	...
	import type { Props as TimeInputProps } from '../inputs/TextInput.svelte'
...
{#if !isEditing}
	...
	>
		...
		<div class="flex items-center gap-1 py-0.5 text-sm tabular-nums">
			<div id={containerID} class={twMerge('w-full', classValue)}>
				...
				{:else if value}
					{value}
				{:else}
					<span class="text-xs text-gray-300"> HH:MM:SS </span>
				{/if}
			</div>
...
{#if isEditing}
	...
	>
		<TimeInput
			bind:inputRef
			bind:value={formState.value as string}
